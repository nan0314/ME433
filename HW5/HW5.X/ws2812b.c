...
#define HIGHTIME 64 // number of 48MHz cycles to be high for 1.65uS
...
void ws2812b_setup() {
    ...
    TRISBbits.TRISB6 = 0;   // initializes B4 as a digital input
    LATBbits.LATB6 = 0;      // turn off the digital output
}
...
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    for (i = 0; i < numLEDs; i++) {
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (c[i].r >> 7) { //identify the bit in c[].r, is it 1
                // the high is longer
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (c[i].g >> 7) { //identify the bit in c[].r, is it 1
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
        ...
        for (j = 7; j >= 0; j--) {
            // if the bit is a 1
            if (c[i].b >> 7) { //identify the bit in c[].r, is it 1
                // the high is longer
                delay_times[nB] = delay_times[nB - 1] + HIGHTIME;
                nB++;
                delay_times[nB] = delay_times[nB - 1] + LOWTIME;
